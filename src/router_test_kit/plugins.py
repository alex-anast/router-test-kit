...
class PluginManager:
    ...
    def _register_builtin_devices(self) -> None:
        ...
        from .device import HostDevice, LinuxDevice, OneOS6Device, RADIUSServer
    ...
    def _load_single_plugin(self, entry_point) -> None:
        ...
        try:
            ...
            logger.info(
                f"Successfully loaded plugin: {plugin_name} -> {device_class.__name__}"
            )
    ...
    def register_device(self, name: str, device_class: Any) -> None:
        ...
        try:
            ...
            self._registered_devices[name] = device_class
            logger.info(
                "Manually registered device: %s -> %s", name, device_class.__name__
            )
    ...
    def create_device(
        self, device_type: str, username: str, password: str, **kwargs
    ) -> Device:
        """
        ...
        if device_class is None:
            ...
            raise PluginError(
                f"Unknown device type: {device_type}. Available types: {available}"
            )
